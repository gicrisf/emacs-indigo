...
(ert-deftest test-bond-mixed-orders ()
  "Test molecule with different bond orders."
  (indigo-with-molecule (mol "C=CC#N")  ; ethylene + nitrile
    (indigo-with-bonds-iterator (bonds-iter mol)
      (let ((orders '())
            (bond (indigo-next bonds-iter)))
        (while bond
...
(ert-deftest test-bond-count-cyclic ()
  ...
  (indigo-with-molecule (mol "C1CCC1")  ; Cyclobutane
    (indigo-with-bonds-iterator (bonds-iter mol)
      (let ((count 0)
            (bond (indigo-next bonds-iter)))
        (while bond
